...
static constexpr bool executionEstUtf8 = sizeof("é") == 3 and "é"[0] == char(0xC3) and "é"[1] == char(0xA9);  //NOTE: Certaines version de gcc donnent l'erreur "error: converting to execution character set: Invalid argument" si ce fichier n'est pas en UTF-8 et qu'on tente d'avoir une encodage d'exécution en latin-1.
